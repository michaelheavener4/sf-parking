...
def binary_metrics(y, p, threshold=.5):
    ...
    return {"n": len(y), "positives": int(y.sum()), "tp": tp, "fp": fp, "fn": fn, "tn": tn,
...
def transition_metrics(y, lag, pred, threshold):
    ...
    pd = pred - lag
    return {"n": int(mask.sum()), "mae": float(np.mean(np.abs(pred[mask] - y[mask]))),
...
def choose_windows(first, latest, train_days, val_days, test_days):
    first_day, last_day = first.date(), latest.date()
    available = (last_day - first_day).days + 1
    td, vd = min(test_days, max(1, available // 4)), min(val_days, max(1, available // 4))
    tr = min(train_days, max(1, available - td - vd))
    test_start = last_day - timedelta(days=td - 1)
    val_start = test_start - timedelta(days=vd)
    train_start = max(first_day, val_start - timedelta(days=tr))
...
def collect_background(conn, start: date, end: date, per_day: int, seed: int, cap: int):
    ...
    while day <= end and len(rows) < cap:
        part = sample_day_targets(conn, day, per_day, seed)
        rows.extend(part)
        print(f"  background {day}: +{len(part):,}; total={len(rows):,}", flush=True)
...
def collect_events(conn, start: date, end: date, per_day: int, seed: int, threshold: float, cap: int):
    ...
    while day <= end and len(rows) < cap:
        part = conn.run(sql, day=day, threshold=threshold, seed=str(seed), limit_rows=per_day)
        rows.extend(part)
        print(f"  enriched   {day}: +{len(part):,}; total={len(rows):,}", flush=True)
        day += timedelta(days=1)
    return rows[:cap]
...
def fit_models(train, val, threshold):
    ...
    common = dict(n_estimators=900, learning_rate=.035, num_leaves=31, min_child_samples=100,
                  subsample=.9, colsample_bytree=.9, reg_alpha=.05, reg_lambda=.2,
                  random_state=42, n_jobs=-1, verbosity=-1)
    yt = (np.abs(train.delta) >= threshold).astype(int)
    yv = (np.abs(val.delta) >= threshold).astype(int)
    event = lgb.LGBMClassifier(objective="binary", **common)
    event.fit(train[FEATURES_SPATIAL], yt, eval_X=val[FEATURES_SPATIAL], eval_y=yv,
    ...
    direction.fit(te[FEATURES_SPATIAL], (te.delta > 0).astype(int), eval_X=ve[FEATURES_SPATIAL], eval_y=(ve.delta > 0).astype(int),
                  callbacks=[lgb.early_stopping(75, verbose=False)])
    magnitude = lgb.LGBMRegressor(objective="regression", **common)
    magnitude.fit(te[FEATURES_SPATIAL], np.abs(te.delta), eval_X=ve[FEATURES_SPATIAL], eval_y=np.abs(ve.delta),
                   callbacks=[lgb.early_stopping(75, verbose=False)])
...
def main():
    ...
    ap.add_argument("--event-enrichment", type=int, default=50000)
    args = ap.parse_args()
    print("🚦 SF PARKING — EVENT-DRIVEN STATE TRANSITION TOURNAMENT")
    conn = connect()
    try:
        first, latest = conn.run("SELECT min(slot_start),max(slot_start) FROM parking_state_hourly WHERE slot_start<=NOW()")[0]
        tr0,v0,t0,t1 = choose_windows(first,latest,args.train_days,args.validation_days,args.test_days)
        print(f"Train {tr0} → {v0-timedelta(days=1)}; Validation {v0} → {t0-timedelta(days=1)}; Test {t0} → {t1}")
        tr_days,va_days,te_days = max(1,(v0-tr0).days),max(1,(t0-v0).days),max(1,(t1-t0).days+1)
        bg = min(50000,max(1000,math.ceil(args.max_train_rows/tr_days)))
        ev = max(500,math.ceil(args.event_enrichment/tr_days))
        print("[1/4] Chronological samples")
        background = collect_background(conn,tr0,v0-timedelta(days=1),bg,61,args.max_train_rows)
        enriched = collect_events(conn,tr0,v0-timedelta(days=1),ev,62,args.transition_threshold,args.event_enrichment)
        seen={(r[0],r[1]) for r in background}; enriched=[r for r in enriched if (r[0],r[1]) not in seen]
        train_targets=background+enriched
        val_targets=collect_background(conn,v0,t0-timedelta(days=1),max(1000,math.ceil(args.max_validation_rows/va_days)),63,args.max_validation_rows)
        test_targets=collect_background(conn,t0,t1,max(1000,math.ceil(args.max_test_rows/te_days)),64,args.max_test_rows)
        ...
        print("[2/4] Leakage-safe features")
    ...
    x=test[FEATURES_SPATIAL]; lag=test.lag1_availability.to_numpy(float); y=test.target.to_numpy(float); actual=y-lag
    event_y=(np.abs(actual)>=args.transition_threshold).astype(int)
    p_event=event.predict_proba(x)[:,1]; p_up=direction.predict_proba(x)[:,1]; mag=np.maximum(0,magnitude.predict(x))
    expected_delta=p_event*(2*p_up-1)*mag; state=np.clip(lag+expected_delta,0,1)
    print("[4/4] Results")
    er=binary_metrics(event_y,p_event); er["roc_auc"]=rank_auc(event_y,p_event); er["pr_auc"]=average_precision(event_y,p_event)
    print("event_detector:",json.dumps(er,sort_keys=True))
    em=event_y==1; da=float(np.mean((actual[em]>0)==(p_up[em]>=.5))) if em.any() else None
    ...
    state_metric=metric(y,state); persist_metric=metric(y,lag)
    report={"version":1,"model":"event_direction_magnitude_v1","transition_threshold":args.transition_threshold,
            "windows":{"train":[str(tr0),str(v0-timedelta(days=1))],"validation":[str(v0),str(t0-timedelta(days=1))],"test":[str(t0),str(t1)]},
            "rows":{"train":len(train),"validation":len(val),"test":len(test),"enriched_training_events":len(enriched)},
            "event_detector":er,"direction_accuracy_on_actual_events":da,"persistence":persist_metric,
            "event_driven_expected_state":state_metric,"transitions":transitions,"feature_count":len(FEATURES_SPATIAL),
            "promotion":"candidate" if state_metric["mae"] < persist_metric["mae"] else "retained_only"}
    OUT.parent.mkdir(parents=True,exist_ok=True); OUT.write_text(json.dumps(report,indent=2,default=str)); print(f"Report: {OUT}")
    return 0
