...
def lm(d): return datetime.combine(d,time.min,TZ)
def win(a,b): return lm(a).astimezone(UTC),lm(b+timedelta(days=1)).astimezone(UTC)
def folds(first,latest,train_days,test_days,max_folds):
    ...
    while end>=fd and len(out)<max_folds:
        ...
        if trs<fd: break
        out.append({"train":win(trs,te),"test":win(ts,end),"local_days":{"train":[str(trs),str(te)],"test":[str(ts),str(end)]}}); end-=timedelta(days=test_days)
    return list(reversed(out))
def metric(y,p):
    e=np.asarray(p,float)-np.asarray(y,float); return {"mae":float(np.mean(np.abs(e))),"rmse":float(np.sqrt(np.mean(e*e))),"bias":float(np.mean(e))}
def mapping_sql():
    return "SELECT DISTINCT post_id::text AS post_id, blockface_id::text AS blockface_id FROM parking_meters WHERE post_id IS NOT NULL AND blockface_id IS NOT NULL"
def targets(conn,start,end,k,seed):
    return conn.run(f"""
      ...
      FROM slots s JOIN capacity c ON c.blockface_id=s.blockface_id WHERE c.capacity>0
      ORDER BY hashtext(s.blockface_id::text||'|'||s.slot_start::text||:seed::text),s.blockface_id,s.slot_start LIMIT :k""",start=start,end=end,seed=str(seed),k=k)
def labels(conn,ts):
    conn.run("DROP TABLE IF EXISTS _bfv3_labels"); conn.run("CREATE TEMP TABLE _bfv3_labels(blockface_id text,slot_start timestamptz,capacity int,local_date date,local_hour int)")
    ...
    rows=conn.run(f"""
      ...
      mapped AS (
        SELECT s.transmission_id,m.blockface_id,s.session_start,s.session_end FROM sessions s JOIN mapping m ON m.post_id=s.post_id::text)
    ...
    conn.run("DROP TABLE IF EXISTS _bfv3_labels"); return rows
def learn(conn,start,end):
    ...
    hist=conn.run(f"""
      ...
      SELECT m.blockface_id,
        ...
        COUNT(DISTINCT t.transmission_id)::double precision AS n
    ...
    drows=conn.run(f"""
      WITH mapping AS ({mapping_sql()})
      SELECT m.blockface_id,EXTRACT(EPOCH FROM (t.session_end-t.session_start))/3600.0 AS duration_h
      FROM meter_transactions t JOIN mapping m ON m.post_id=t.post_id::text
      WHERE t.session_start>=:start AND t.session_start<:end AND t.session_end IS NOT NULL AND t.session_end>t.session_start""",start=start,end=end)
    by_block={}; all_d=[]
    for r in drows:
...
def survival_prob(durs,age,extra):
    ...
    denom=max(int(np.sum(durs>age)),1); return float(np.sum(durs>age+extra)/denom)
def forecast(conn,ts,rates,global_rate,by_block,global_durs):
    conn.run("DROP TABLE IF EXISTS _bfv3_pred"); conn.run("CREATE TEMP TABLE _bfv3_pred(blockface_id text,target_slot timestamptz,capacity int,local_date date,local_hour int)")
    import csv; from io import StringIO
    b=StringIO(); csv.writer(b,lineterminator='\n').writerows(ts)
    ...
    starts=conn.run(f"""
      WITH mapping AS ({mapping_sql()})
      SELECT p.blockface_id,p.target_slot,EXTRACT(EPOCH FROM ((p.target_slot-INTERVAL '1 hour')-t.session_start))/3600.0 AS age_h
      FROM _bfv3_pred p JOIN mapping m ON m.blockface_id=p.blockface_id
      JOIN meter_transactions t ON t.post_id=m.post_id::text
      WHERE t.session_start<=p.target_slot-INTERVAL '1 hour' AND t.session_start>p.target_slot-INTERVAL '73 hours'""")
    by_target={}
    ...
    for bf,slot,cap,local_date,local_hour in ts:
        ...
        lam=float(rates.get((str(bf),int(local_date.isoweekday()),int(local_hour)),global_rate)); new=0.0
        for j in range(12):
            remaining=1.0-(j+0.5)/12.0; surv=float(np.mean(durs>remaining)) if len(durs) else math.exp(-remaining/1.5); new+=(lam/12.0)*surv
        expected=max(0.0,existing+new); out.append(1.0-min(1.0,expected/max(int(cap),1)))
    conn.run("DROP TABLE IF EXISTS _bfv3_pred"); return np.asarray(out,float)
def main():
