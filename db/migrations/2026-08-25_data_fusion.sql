...
CREATE INDEX IF NOT EXISTS idx_sfpark_sensor_hourly_time
    ON sfpark_sensor_hourly(start_time_local);
CREATE INDEX IF NOT EXISTS idx_sfpark_sensor_hourly_block
    ON sfpark_sensor_hourly(street_block, start_time_local);
    ...
    SELECT
        ...
        CASE WHEN (s.total_occupied_time + s.total_vacant_time) > 0
             THEN s.total_occupied_time::double precision /
                  (s.total_occupied_time + s.total_vacant_time)
        END AS sensor_total_occupancy,
        CASE WHEN (s.gmp_occupied_time + s.gmp_vacant_time) > 0
             THEN s.gmp_occupied_time::double precision /
                  (s.gmp_occupied_time + s.gmp_vacant_time)
        END AS sensor_gmp_occupancy,
        s.rate,
        s.rate_type
...
FROM sensor s
FULL OUTER JOIN tx
  ON tx.local_hour = s.local_hour
 AND tx.street_block = s.street_block;
...
SELECT
    ...
    sensor_total_occupancy,
    sensor_gmp_occupancy,
    rate,
    rate_type
FROM v_fusion_sensor_transaction_hourly
