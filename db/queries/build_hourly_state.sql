...
-- at the forecast timestamp.  We materialize zero-transaction hours explicitly
-- so "no paid use" is a real state, not a missing row.
WITH hours AS (
    SELECT gs AS slot_start
    FROM generate_series(
        :day_start,
        :day_end - INTERVAL '1 hour',
        INTERVAL '1 hour'
    ) gs
),
posts AS (
    SELECT post_id
    FROM _state_post_spans
    WHERE first_local_date <= (:day_start AT TIME ZONE 'America/Los_Angeles')::date
      AND last_local_date  >= (:day_start AT TIME ZONE 'America/Los_Angeles')::date
),
grid AS (
